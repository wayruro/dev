<style type="text/css">
  body {
  margin: 0;
  font-family: "Myriad Set Pro","Helvetica Neue","Helvetica","Arial",sans-serif;
  font-size:100%;
  line-height:1.5;
  height:100%;
  color: #fff;
  border-top: 2px solid #00a0d2;
  background: #456878 url("data:image/png;base64,");
  background-attachment: fixed;
  background-size: contain;
  background-size: 100% 100%;
  box-sizing: border-box;
}

#login {
    width: 300px;
    height: 300px;
    padding: 10px;
    position: absolute;
    top: 50%;
    left: 50%;
    margin-top: -150px;
    margin-left: -150px;
}

h1 {
  text-shadow: 0.5px 0.5px 2px rgba(0, 0, 0, 0.3);
}

#login:after {
    content: 'Site Crafted in Orlando - Florida';
    text-align: center;
    text-indent: 0 !important;
    display: block;
    width: 100%;
    font-size: 0.8rem;
    color: #5F7883;
    position: relative;
    bottom: -50%;
}
</style>

<div id="login" class="content-area" style="text-align: center;">

    <main id="main" class="site-main" role="main">

      <h1 style="color: #fff; font-size: 1.75rem; font-weight: 300; letter-spacing: 1px;">Be Ready, We Are Launching Soon.</h1>

    </main>

</div>



